...
bot = telebot.TeleBot('5492022827:AAGdlRm7iCn5V_NfxvPlPNtNYy0kr4JT9FQ')
...
def bot_vasy(message):
    ...
    if k < candies:
        candies -= k
        bot.send_message(message.chat.id, f'Бот Вася забрал {k} конфет. На столе осталось {candies} конфет.\nВаш ход, {player}. Сколько конфет Вы хотите забрать?')
    else:
        bot.send_message(message.chat.id, 'Бот Вася выиграл!')
...
def start(message):
    ...
    if flag:
        ...
        bot.send_message(message.chat.id, f'На столе {candies} конфет. Бот Вася делает первый ход')
        bot_vasy(message)
